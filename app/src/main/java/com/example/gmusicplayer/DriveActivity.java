...
public class DriveActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.play_btn).setOnClickListener(view -> startPlaying(t_fileId));
        findViewById(R.id.stop_btn).setOnClickListener(view -> stopPlaying());
     ...
     * Playing Audio
     */
    String t_fileId = null; File t_file = null;
    boolean isPLAYING = false;
    MediaPlayer mp = null;

    private void startPlaying(String fileId) {
        if (mDriveServiceHelper != null) {
            Log.d(TAG, "Playing file " + fileId);

            if (!isPLAYING) {
                isPLAYING = true;
                mp = new MediaPlayer();

                mDriveServiceHelper.getLink(fileId)
                        .addOnSuccessListener(fileLink -> {
                            try {
                                mp.setDataSource(fileLink);
                                mp.prepare();
                                mp.start();
                            } catch (IOException e) {
                        ...
                        .addOnFailureListener(exception -> Log.e(TAG, "Couldn't Play file.", exception));
            }
        }
    }
